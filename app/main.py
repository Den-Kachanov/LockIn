...
class StudyCompleteRequest(BaseModel):
    ...
    started_at: Optional[str] = None   # ISO string from frontend
...
@api_router.post("/study/complete", response_model=StudyCompleteResponse)
def study_complete(data: StudyCompleteRequest, request: Request, db: Session = Depends(get_db)):
    """
    Called by the frontend when a Pomodoro study phase finishes (25 min).

    - Logs a StudySession record
    - Awards 10 stars (points) to the user
    - Updates total_study_minutes and streak
    """
    STARS_PER_SESSION = 10
    ...
    now = datetime.utcnow()
    try:
        started_at = datetime.fromisoformat(data.started_at.replace("Z", "+00:00")).replace(tzinfo=None) if data.started_at else now
    except Exception:
        started_at = now
    ...
    # Award stars
    db_user.points = (db_user.points or 0) + STARS_PER_SESSION

    # Update total study minutes
    db_user.total_study_minutes = (db_user.total_study_minutes or 0) + data.duration_minutes

    # Update streak (if last study date is not today)
    today_str = now.date().isoformat()
    if db_user.last_study_date != today_str:
        yesterday_str = (now.date() - timedelta(days=1)).isoformat()
        if db_user.last_study_date == yesterday_str:
            db_user.current_streak = (db_user.current_streak or 0) + 1
        else:
            db_user.current_streak = 1  # streak broken, restart
        db_user.last_study_date = today_str
    ...
    return StudyCompleteResponse(
        ...
        stars_earned=STARS_PER_SESSION,
