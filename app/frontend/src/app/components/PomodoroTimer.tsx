...
const sendStudySession = async (minutes: number) => {
  try {
...
export function PomodoroTimer() {
  ...
  const [studyTime] = useState(25 * 60); // 25 хвилин
  const [breakTime] = useState(5 * 60);  // 5 хвилин
    ...
    const interval = setInterval(() => {
      setCurrentTime((prev) => {
        if (prev <= 1) {
          ...
          // Відправка на сервер сесії
          if (isStudying) sendStudySession(studyTime / 60); // з секунд у хвилини

          setIsStudying(nextIsStudying);
          return nextIsStudying ? studyTime : breakTime;
        }
        return prev - 1;
      });
    }, 1000);
  ...
  }, [isRunning, isStudying, studyTime, breakTime]);
  ...
  const progressPercent = (currentTime / (isStudying ? studyTime : breakTime)) * 100;

  return (
    <div className="relative">
      {/* Glow overlay */}
      <motion.div
        className="absolute -inset-4 pointer-events-none bg-gradient-to-r from-[#00d9ff]/30 via-[#ff00ff]/30 to-[#ffd700]/30 rounded-3xl blur-xl"
      ...
      <div className="relative bg-gradient-to-br from-[#1a1f3a]/90 to-[#2d1b4e]/90 backdrop-blur-xl border-2 border-[#00d9ff]/50 rounded-3xl p-8 shadow-2xl">
        {/* Glitch overlay */}
        <motion.div
          className="absolute inset-0 pointer-events-none bg-gradient-to-r from-[#00d9ff]/10 to-[#ff00ff]/10 rounded-3xl"
        ...
        <div className="text-center mb-8">
          ...
          <div className="relative w-full h-3 bg-black/50 rounded-full overflow-hidden">
            <motion.div
              ...
              animate={{
                boxShadow: ['0 0 10px #00d9ff', '0 0 20px #ff00ff', '0 0 10px #ffd700'],
              }}
              transition={{ duration: 2, repeat: Infinity }}
            />
          </div>
        ...
        <div className="flex gap-4 justify-center">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            animate={{
              boxShadow: ['0 0 20px #00d9ff', '0 0 30px #ff00ff', '0 0 20px #00d9ff'],
            }}
