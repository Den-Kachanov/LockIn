...
function SingleSlotMachine({ id, betAmount, onSpinComplete, disabled }: SlotMachineProps) {
  ...
  const spin = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/casino/spin', {
        ...
        credentials: 'include',
        body: JSON.stringify({ bet_amount: betAmount, machine_id: id }),
      });

      if (!res.ok) {
        const err = await res.json();
        setError(err.detail || 'Spin failed');
        setIsSpinning(false);
      ...
      // Animate slots stopping one by one with server result
      const spinDurations = [1000, 1500, 2000];
      spinDurations.forEach((duration, index) => {
        setTimeout(() => {
          setSlots((prev) => {
            const updated = [...prev];
            updated[index] = result.slots[index];
            return updated;
          });
              ...
              >
                <div className="absolute inset-0 flex flex-col items-center justify-start">
                  {spinningSlots[i] ? (
                    ...
                    >
                      {[...Array(20)].map((_, idx) => {
                        const randomSymbol = slotSymbols[idx % slotSymbols.length];
                        const RandomIcon = randomSymbol.icon;
                        return (
                          <div key={idx} className="w-20 h-20 md:w-28 md:h-28 flex items-center justify-center flex-shrink-0">
                            <RandomIcon className="w-10 h-10 md:w-14 md:h-14" style={{ color: randomSymbol.color }} />
                          </div>
                        );
                      })}
                    </motion.div>
                    ...
                    >
                      <Icon className="w-10 h-10 md:w-14 md:h-14" style={{ color: symbol.color }} />
                    </motion.div>
                  )}
                </div>
...
export function Casino() {
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch('/api/casino/stats', { credentials: 'include' });
      if (res.ok) {
        const data = await res.json();
        setCasinoStats(data);
      }
    } catch (e) {
      console.error('Failed to fetch casino stats:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSpinComplete = (result: SpinResult) => {
    setCasinoStats((prev) => ({
      ...
      total_winnings: prev.total_winnings + result.win_amount,
      spins_today: prev.spins_today + 1,
    }));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Points', value: casinoStats.total_points.toLocaleString(), icon: '⭐', color: '#ffd700' },
          { label: 'Total Winnings', value: casinoStats.total_winnings.toLocaleString(), icon: '💰', color: '#00d9ff' },
          { label: 'Spins Today', value: casinoStats.spins_today, icon: '🎰', color: '#ff00ff' },
          { label: 'Win Rate', value: `${casinoStats.win_rate}%`, icon: '📊', color: '#00d9ff' },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
      ...
      >
        ...
        <div className="relative">
          <motion.h2
            ...
            animate={{
              textShadow: ['0 0 20px #ffd700', '0 0 40px #ff00ff', '0 0 20px #00d9ff'],
            }}
            transition={{ duration: 2, repeat: Infinity }}
          >
            🎰 LOCK IN CASINO 🎰
          ...
          <div className="flex gap-4 justify-center">
            <div className="px-4 py-2 bg-black/30 rounded-lg border border-[#ffd700]/30">
              ...
              <span className="text-lg text-[#ffd700] font-bold">{casinoStats.total_points.toLocaleString()} ⭐</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <SingleSlotMachine id={1} betAmount={50} onSpinComplete={handleSpinComplete} disabled={loading} />
          <SingleSlotMachine id={2} betAmount={100} onSpinComplete={handleSpinComplete} disabled={loading} />
          <SingleSlotMachine id={3} betAmount={200} onSpinComplete={handleSpinComplete} disabled={loading} />
        </div>
      </div>
