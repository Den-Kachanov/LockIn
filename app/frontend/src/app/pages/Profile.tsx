...
export function Profile() {
  ...
  const handleSaveUsername = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setEditError(data.detail || 'Failed to update username');
        return;
      }

      // Only update the username locally if the server returned a new token
      if (data.new_token) {
        ...
        setEditError('Server did not return new token; username not updated locally.');
      }
    } catch {
